...
- 具体例
  ...
  ```bash
  # 長い時間がかかる処理をバックグラウンドで実行することでコマンドが終了するのを待たなくてよくなる
  $bg %1
  ```
...
# またマニュアルに戻る（ジョブズコマンドの番号の1)
# さきほどのスクロールしていたところに戻ることができる
$fg %1
```

---

## 50 【killコマンド】プロセス・ジョブを終了しよう
...
- 具体例
  ...
  ```bash
  # フォアグラウンドのジョブはcommand+cで終了できる
  # 停止中のバッググラウンドのジョブはkillコマンドで終了できる
  $kill %1
  $kill 4965
  ```
...
- 補足
  ...
  - デフォルトでは、TERMという、終了を支持するシグナルを送信している
...
- できるようになったこと
  - プロセスとジョブについて理解した
  - プロセスを確認できるようになった
  - ジョブをバックグラウンド・フォアグラウンドで実行できるようになった
  - ジョブやプロセスを終了できるようになった
  
- やってみよう
  - yes > /dev/null & コマンドを実行してCPU負荷をかけてみよう

    ```bash
    $yes > /dev?null &
    ```

  - 上記のyesコマンドのプロセスIDを調べよう
...
- 本講座のまとめ
  - できるようになったこと
    - Linuxの必須コマンドを使えるようになった
    - Linuxの仕組みについて理解した
    - Linuxの考え方が身についた
  - 今後に向けて
    - 普段からターミナルを使おう
    - LinuCレベル1もしくはLPIC-1を受講しよう
    - 「UNIXという考え方」を読もう
    - Linuxをパソコンにインストールしてみよう
